...
function hasMediaContext(state) {
  ...
  return state.state === 'playing' || state.state === 'paused' ||
    (state.state === 'idle' && state.attributes.media_title);
}
...
// ── Speaker Card ────────────────────────────────────────────────
let _volTimer = null;
function SpeakerCard({ entityId, state, isSelected, isGrouped, onTap, hass }) {
  const info = getSpeakerInfo(entityId, state);
  const haGroup = (state.attributes.group_members || []).length > 1;
  ...
  return html`
    <div class=${cls} onClick=${() => onTap(entityId)}>
      <p class="smc-speaker-name">${info.name}${haGroup && !isGrouped && !isSelected
      ...
      <div class="smc-speaker-grid">
        ${maPlayers.map(id => {
          ...
          return html`<${SpeakerCard} key=${id} entityId=${id} state=${state}
            isSelected=${idx === 0} isGrouped=${idx > 0} onTap=${onSelect} hass=${hass} />`;
        })}
      </div>
    </div>
