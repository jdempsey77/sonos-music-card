...
## Current version
**v0.22.2** — **Road Trip Radio metadata fix.** Road Trip Radio (CH 301, SiriusXM
UUID `c3890f3c`) shows `media_channel` as the Now Playing title instead of
`media_title` — SXM injects sponsor/ad text into this channel's broadcast metadata
stream during songs. Detection is scoped to the UUID `c3890f3c` only (in
`buildNpInfo`'s Sonos-native branch); `sid=37` was dropped in v0.22.2 because it
matched multiple SiriusXM channels. All other SXM channels keep showing live metadata.
